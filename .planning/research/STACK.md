...
**Domain:** Elastic GPU Telemetry Pipeline — four Go microservices on Kubernetes with custom in-memory MQ
**Researched:** 2026-06-27
**Confidence:** MEDIUM (versions verified via pkg.go.dev and GitHub releases; architectural choices cross-checked against community usage)
...
Use buf CLI with **remote BSR plugins** — no local protoc-gen-go or protoc-gen-go-grpc install needed:
...
managed:
  ...
  override:
    - file_option: go_package_prefix
      value: github.com/ajitg/vantage/pkg/pb
plugins:
...
The `require_unimplemented_servers=false` opt prevents compile failures when adding new RPC methods incrementally during development.
...
**For the MQ gRPC server (Produce + Consume streaming):**
...
**For the Collector batch insert pattern:**
...
**For the API Gateway annotation pattern (swag):**
...
**For the Docker multi-stage build pattern:**
```dockerfile
...
**For the race-detector MQ tests:**
...
---
*Stack research for: Elastic GPU Telemetry Pipeline (vantage) — Go microservices on Kubernetes*
*Researched: 2026-06-27*
