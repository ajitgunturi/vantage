...
3. **Collector Microservice:** Connects to the Custom MQ via a persistent gRPC Server-Side Stream. It consumes data as it arrives, parses the payload, and persists it into a PostgreSQL database.
...
- **Dual-Protocol Transport Layer:**
  - **gRPC Interface (Data Plane):** Provide a unary RPC for Streamers to push data (`Produce`), and a **Server-Side Streaming RPC** (`Consume`) for Collectors. 
...
- **Delivery Semantics:** Decoupled, thread-safe queueing. Ensure multiple Collectors pulling from the streaming endpoint receive unique messages.
...
- **Ingestion:** Establish a long-lived connection to the MQ gRPC stream. Process incoming metrics reactively.
