<!-- The SINGLE project instruction file (lean by design; depth lives in .planning/). Hand-curated,
     intentionally has NO `<!-- GSD:* -->` markers so `generate-claude-md` treats it as hand-crafted
     and leaves it untouched (won't re-inject duplicated stack/architecture sections). Do not
     reintroduce a root CLAUDE.md or a generated .claude/CLAUDE.md alongside this one. -->
...
## GSD workflow
This project is GSD-managed; route work through a GSD command so `.planning/` stays in sync. Per-phase
entry: `/gsd-spec-phase N` → `/gsd-plan-phase N` → `/gsd-execute-phase N`. For small fixes/doc tweaks
use `/gsd-quick`; for investigation use `/gsd-debug`. Avoid direct repo edits outside a GSD workflow
unless explicitly bypassing. (`/gsd-profile-user` optionally generates a developer profile.)
